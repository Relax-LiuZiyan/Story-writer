...
#### 开启两个打流对虚拟网卡
该打印测试信息只纯在于虚拟网卡接收的有效数据报文，本次测试在C端和S端同时开了两个打流软件，都是如下图测试结果相同（无论打流绑定的CPU是否为同一个），有趣的现象在于==三个Enter同时出现的情况 #F44336==。针对于此情况，可以初步认为：对于同一个物理网卡可能采用排列入协议栈函数（因为此驱动使用了三条物理链路，也就是三个物理网卡），==每个网卡都是一个独立的进程进行调度进入协议栈函数 #F44336==。
