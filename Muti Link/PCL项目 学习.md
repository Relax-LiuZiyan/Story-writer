...
1. [CPU性能设置](#cpu性能设置)
	...
	2. [查看CPU信息](#查看cpu信息)
		1. [CPU支持主频](#cpu支持主频)
		2. [CPU主频可设置的值](#cpu主频可设置的值)
		3. [调节策略](#调节策略)
		4. [模式使用](#模式使用)
	3. [命令总结](#命令总结)
	4. [编写cpu_set.sh脚本](#编写cpu_setsh脚本)
	5. [CPU隔离](#cpu隔离)
		1. [用户态隔离](#用户态隔离)
		2. [内核态隔离](#内核态隔离)
		3. [内核态线程](#内核态线程)
	6. [常用关于CPU的命令](#常用关于cpu的命令)
	7. [参考](#参考)
2. [常用打流命令](#常用打流命令)
	1. [iperf3](#iperf3)
	2. [iperf](#iperf)
	3. [打流软件对比](#打流软件对比)
	4. [额外知识](#额外知识)
3. [LINUX内核线程的使用](#linux内核线程的使用)
	1. [kthread_create() 创建内核线程](#kthread_create-创建内核线程)
	2. [kthread_run() 创建并运行内核线程](#kthread_run-创建并运行内核线程)
	3. [kthread_stop()，停止指定内核线程](#kthread_stop停止指定内核线程)
	4. [kthread_should_stop()，判断线程是否该停止](#kthread_should_stop判断线程是否该停止)
	5. [kthread_bind()，绑定创建好的线程在执行CPU核心上运行](#kthread_bind绑定创建好的线程在执行cpu核心上运行)
		1. [编译驱动时提示warning : "kthread_create_on_cpu" undefined!](#编译驱动时提示warning-kthread_create_on_cpu-undefined)
		2. [参考](#参考)
	6. [实际测试](#实际测试)
	7. [注意](#注意)
	8. [查看线程的CPU利用率](#查看线程的cpu利用率)
	9. [参考](#参考)
4. [LINUX内核任务延迟队列](#linux内核任务延迟队列)
5. [LINUX内核定时器](#linux内核定时器)
6. [LINUX内核延时函数](#linux内核延时函数)
	1. [忙等待短延时](#忙等待短延时)
	2. [忙等待长延时函数](#忙等待长延时函数)
	3. [睡眠短延时](#睡眠短延时)
		1. [sleep类延时函数](#sleep类延时函数)
		2. [schedule类睡眠延时函数](#schedule类睡眠延时函数)
		3. [sleep_on类，在等待队列上睡眠的延时函数](#sleep_on类在等待队列上睡眠的延时函数)
	4. [参考](#参考)
7. [常用的网站](#常用的网站)
