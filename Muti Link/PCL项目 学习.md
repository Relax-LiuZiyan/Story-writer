...
#### 测试2：开启两个打流对虚拟网卡
该打印测试信息只纯在于虚拟网卡接收的有效数据报文，本次测试在C端和S端同时开了两个打流软件，都是如下图测试结果相同（无论打流绑定的CPU是否为同一个），有趣的现象在于==三个Enter同时出现的情况 #F44336==。针对于此情况，可以初步认为：对于同一个物理网卡可能采用排列入协议栈函数（因为此驱动使用了三条物理链路，也就是三个物理网卡），==每个网卡都是一个独立的进程进行调度并行进入协议栈函数 #F44336==。

![测试代码](./images/1667980430830.png)

![ 测试结果](./images/1667980408075.png)
#### 测试3：开启两个打流对同一个物理网卡
使用两个iperf3终端打流S端同一个物理网卡，每条流100M， 打了10S左右，保留了打印信息3W条，即1.5W个报文，测试代码在hook处加了50us的延时，但是测试结果没有出现连续两个ENTER，==即所有的报文全是排队进入网络协议栈函数 #F44336==。
...
## 内存函数申请最大值
==**此处不考虑巨页的使用** #F44336==，使用下方代码可以测试出当前系统内核可以申请的最大内存为4MB，对应64位系统指针大小位8B，即可以放下4MB/8B=524,288个报文，每个报文为1500B，对应为524288\*1500\*8=5.86Gb，对应一个10Gb网卡来说可以使用。
> ==**注意** #F44336==：虽然申请的内存可以存放5.86G的报文数据，但如果消费者线程处理能力小于生产者线程能力，即会造成数据的不断叠加，最终导致内存满而释放报文，导致丢包。因此该申请的内存可以用于短时间数据报文的突发，但要保证消费者线程处理报文的速度大于生产者线程生产报文的速度。
``` c?linenums
...
if(!str){
    ...
    DBG("kmalloc create successfully");
}
```
# 常用的网站
