...
# 主界面介绍
上面界面是可以通过设置进行修改，条状为CPU占用率，下侧界面为任务线程(进程、用户态线程与内核线程等)。

![htop主界面](./images/1667393961269.png)
## 主界面上册介绍

![使用率与颜色对应](./images/1667394059553.png)
...
![推荐使用界面](./images/1667393483774.png)
## meters 计量信息
该设置信息可以将每个信息放在不同的位置上，可以通过空格选择显示的方式，回车键添加，del键删除。

![选择信息](./images/1667393535153.png)

| Available meters                         | 详细介绍                                                 |
| ---------------------------------------- | -------------------------------------------------------- |
| Clock                                    | 当前系统时间                                             |
| load averages: 1min, 5mins, 15 mins      | 分别在1，5，15分钟内的CPU平均使用率（在1.2.2有详细介绍） |
| load : averages of ready process in last | 最后一分钟就绪进程的CPU平均使用率                        |
| Tasks counter                            | 任务统计（1.2.1详细介绍）                                |
| Uptime                                   | 系统运行时间                                             |
| Battery                                  | 电量                                                     |
| Hostname                                 | 主机名称                                                 |
| Blank                                    | 占位条，无实际作用                                       |
...
![配置信息](./images/1667393846406.png)
## Columns
在默认条件下，只添加了PPID参数。

![配置信息](./images/1667393794517.png)
# Reference 
