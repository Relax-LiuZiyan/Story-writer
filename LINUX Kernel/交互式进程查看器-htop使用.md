...
### PID/PPID/USER
这部分每个 row 都是一个 process，而 PID 就是每个 process 的 ID。那知道 PID 之后可以干嘛呢？其实他的用途挺多的，譬如说你可以用 kill -KILL <pid> 来杀掉某个 process；或是使用 kill -STOP 来暂停 process 再用 kill -CONT 让他继续执行（这超好用 👍，但好像很多人都不知道）