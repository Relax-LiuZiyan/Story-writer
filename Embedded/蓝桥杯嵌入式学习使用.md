...
### keil编程
在编程实现中添加了`key_flag`变量作为循环检测标志，具体函数实现会进行视频讲解，请注意听课！`key_scan`函数实现了，按键按下后只会返回一次按键值，即按下无论多久函数只返回一次按键值，只有当松开再次按下才会返回按键值。在`main.c`文件中实现B1按下LED1亮，B2按下LED1灭，B3按下
``` c?linenums
