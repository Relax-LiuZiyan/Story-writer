...
在机械按键的触点闭合和断开时，都会产生抖动，为了保证系统能正确识别按键的开关，就必须对按键的抖动进行处理。抖动时间的长短由按键的机械特性决定，一般为5ms～10ms。按键抖动会引起一次按键被误读多次。为确保CPU对键的一次闭合仅作一次处理，必须去除键抖动。去除按键机械抖动的方式很多，比如硬件中的施密特触发器，
