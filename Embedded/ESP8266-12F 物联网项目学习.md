...
void ICACHE_FLASH_ATTR user_init(void)
{
	...
	// OLED显示初始化
	//------------------------------------------------------------
	OLED_Init();							// OLED初始化![enter description here](./images/1656124025461.jpg)
...
void ICACHE_FLASH_ATTR OS_Timer_SNTP_cb(void	 * arg)
{
	...
	char * Str_Head_Year;		// 【"年份"】字符串首地址
	 ...
	 if(TimeStamp)		// 判断是否获取到偏移时间
	 {
		 ...
		 // 【实际时间】字符串 == "周 月 日 时:分:秒 年"  Fri Jun 24 20:52:30 2022
		 os_printf("\r\n----------------------------------------------------\r\n");
		 ...
		 Str_Head_Year -= 5;							// 获取【"年份"】字符串的首地址,【注：API返回的实际时间字符串，最后还有一个换行符，所以这里 -5】
		 T_A_Str_Data[4] = ' ' ;
...
![MQTTFX连接设置](./images/1656317199754.png)![enter description here](https://markdown.xiaoshujiang.com/img/spinner.gif "[[[1656317217714]]]" )
# EPS8266字符串API
